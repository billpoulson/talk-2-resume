...
export class DynamicDatabase {
  ...
  initialData(): DynamicFlatNode[] {
    return this.rootLevelNodes.map(name => new DynamicFlatNode(newUUID(), name, 0, true))
  }
    ...
    setTimeout(() => {
      if (expand) {
        const nodes = children.map(
          name => new DynamicFlatNode(newUUID(), name, node.level + 1, this._database.isExpandable(name)),
        )
